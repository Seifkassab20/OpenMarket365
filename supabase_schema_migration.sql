-- Supabase migration generated from the uploaded database ERD.
-- IMPORTANT:
-- 1) The source is an image, so a few column names/types were visually inferred.
-- 2) Run this in Supabase SQL Editor on a NEW/EMPTY project first.
-- 3) Review the "ERD-INFERRED" comments before using in production.
-- 4) auth.users is Supabase Auth. profiles.id references auth.users(id).
...
create table if not exists public.profiles (
  ...
  country_code varchar(3), -- ERD: ISO alpha-3
...
create table if not exists public.companies (
  ...
  cr_number varchar(100) unique, -- ERD: Commercial Registration
...
-- =========================================================
-- SEED: OPTIONAL EXAMPLE CATEGORIES
-- =========================================================
-- Add your real categories after confirming the ERD data.
-- Example:
-- insert into public.categories (code, name_en, name_ar, hs_code)
-- values ('FOOD', 'Food & Agriculture', 'الأغذية والزراعة', null)
-- on conflict (code) do nothing;

-- =========================================================
-- ERD REVIEW NOTES
-- =========================================================
-- The uploaded ERD image has low-resolution text. Before production,
-- compare these visually inferred fields with the original schema:
--
-- companies:
--   quality_iso, annual_capacity_ml, export_port_history,
--   youtube_video_id, featured_gallery_url, sorting_machinery,
--   cold_storage_capacity_ml, featured_gallery_url_2
--
-- subscription_plans:
--   annual_price_usd_discounted, max_storage_mb,
--   max_video_duration_minutes, monthly_frequency_days
--
-- subscriptions:
--   payment_method, payment_reference, amount, tax,
--   billing_document_url, payment_notes
--
-- products:
--   harvest_season_from/to, packaging_type, price,
--   minimum_order_quantity
--
-- market_listings:
--   description_country_code, inspection_company, approval_at
--
-- content_items:
--   content_type/category/media fields
--
-- The relational structure and FK directions are based on the visible
-- connectors in the supplied ERD.
