...
export default function LoginPage() {
  ...
  const [selectedRoleForLogin, setSelectedRoleForLogin] =
    useState<UserRoleType>('EXPORTER');
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  const ArrowIcon = direction === 'rtl' ? ArrowLeft : ArrowRight;

  // Direct login - still used for Visitor
  ...
  // Switch to Custom Credentials and prefill the selected persona
    ...
    <div className="py-16 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto space-y-10">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-3 pb-6 border-b border-[#b9aa95]">
        ...
        <div className="text-[10px] font-bold text-[#9b452f] uppercase tracking-[0.22em] flex items-center justify-center gap-1.5">
          ...
          <span>09 / ACCESS GATEWAY • 3 QUICK PERSONAS</span>
        </div>

        <h1 className="text-4xl sm:text-6xl font-serif text-[#202522] tracking-tight">
          {language === 'ar'
            ? 'بوابة الدخول الموحدة.'
            : 'Sign in to Market 365.'}
        </h1>

        <p className="text-sm text-[#70695f] leading-relaxed">
          {language === 'ar'
            ? 'سجل دخولك كمصدر مصري، أو مستورد دولي، أو مسؤول رقابي. ويمكنك أيضاً التصفح كزائر.'
            : 'Access the gateway as an Egyptian Exporter, Global Importer, or Platform Admin. You can also continue as a visitor.'}
        </p>
        ...
        <div className="inline-flex p-1 bg-[#e4dac9] border border-[#b9aa95] mt-4">
          ...
          >
            {language === 'ar'
              ? 'بيانات مخصصة (البريد وكلمة السر)'
              : 'Custom Credentials'}
          </button>
      ...
      {activeTab === 'quick' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {/* Persona 2: Global Importer / Buyer */}
            ...
            <div className="pt-6 mt-6 border-t border-[#b9aa95]">
              ...
              <button
                onClick={() =>
                  prefillCredentials(
                    'ADMIN',
                    'compliance@openmarket365.gov.eg'
                  )
                }
                className="w-full py-2.5 text-xs font-bold uppercase tracking-wider bg-[#9b452f] hover:bg-[#833824] text-white transition-colors flex items-center justify-center gap-1.5 shadow-sm"
              >
        ...
        <div className="max-w-md mx-auto bg-[#e4dac9] border border-[#b9aa95] p-8 shadow-sm">
          ...
          <div className="mb-6 text-center pb-4 border-b border-[#b9aa95]">
            ...
            <p className="text-xs text-[#70695f] mt-1">
              {language === 'ar'
                ? 'حدد الدور وادخل البريد التجاري وكلمة المرور'
                : 'Select role and authenticate via Supabase Auth'}
            </p>
          ...
          <div className="mb-4">
            ...
            <label className="block text-[10px] font-bold uppercase tracking-wider text-[#70695f] mb-1.5">
              {language === 'ar'
                ? 'اختر الحساب للتعبئة السريعة:'
                : 'Quick Select Account Persona:'}
            </label>
          ...
          >
            ...
            <div>
              ...
              <div className="flex justify-between items-center mb-1">
                ...
                <a
                  href="#"
                  className="text-[10px] text-[#9b452f] hover:underline font-bold uppercase tracking-wider"
                >
                  {language === 'ar'
                    ? 'نسيت كلمة المرور؟'
                    : 'Forgot?'}
          ...
          <div className="mt-6 text-center pt-6 border-t border-[#b9aa95] text-xs text-[#565047]">
            ...
            <span>
              {language === 'ar'
                ? 'أو تصفح بدون تسجيل:'
                : 'Or explore with no login:'}
            </span>

            <button
              type="button"
              onClick={() =>
                handleQuickLogin('VISITOR', '/products')
              }
      ...
      <div className="text-center pt-6 text-xs text-[#70695f] border-t border-[#b9aa95]">
        ...
        <span>
          {language === 'ar'
            ? 'ليس لديك حساب بعد؟'
            : "Don't have an export or buyer account yet?"}
        </span>

        <Link
          href="/auth/register"
          className="text-[#9b452f] font-bold hover:underline uppercase tracking-wider"
        >
          {language === 'ar'
            ? 'سجل كمصدر مصري أو مشتري دولي الآن'
            : 'Create an Account'}
        </Link>
